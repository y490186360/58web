...
function resolveCSS(htmlStr) {
    ...
    const newCSS = regStyle.exec(htmlStr) // 正泽传进来的参数 存入 新的
    // 4.2 路径
...
function resolveJS(htmlStr) {
    const newJS = regScript.exec(htmlStr)
...
function resolveHTML(htmlStr) {
    // 6.1 正泽 replace()
    const newHtml = htmlStr.replace(regStyle, '<link rel="stylesheet" src="./clock/index.css"/>').replace(regScript, '<script src="./clock/index.js"></script>')
